test_that("HPC shortcuts honor configurable sampling ratios", {
  ...
  ada_low <- sby_adasyn_hpc(dat, y ~ ., sby_adasyn_k = 3,
                            sby_adasyn_ratio = 0.5)
  ada_high <- sby_adasyn_hpc(dat, y ~ ., sby_adasyn_k = 3,
                             sby_adasyn_ratio = 1)
  ...
  near_loose <- sby_nearmiss_hpc(dat, y ~ ., sby_nearmiss_k = 3,
                                 sby_nearmiss_ratio = 0.5)
  expect_equal(nrow(near_equal), 8L)
  expect_equal(nrow(near_loose), 6L)

  adanear_equal <- sby_adanear_hpc(dat, y ~ ., sby_adasyn_k = 3,
                                   sby_nearmiss_k = 3,
                                   sby_adasyn_ratio = 0.5,
                                   sby_nearmiss_ratio = 1)
  adanear_loose <- sby_adanear_hpc(dat, y ~ ., sby_adasyn_k = 3,
                                   sby_nearmiss_k = 3,
                                   sby_adasyn_ratio = 1,
                                   sby_nearmiss_ratio = 0.5)
...
test_that("sby_adanear_hpc honors fixed seeds", {
  ...
  out1 <- sby_adanear_hpc(dat, y ~ ., sby_adasyn_k = 3,
                          sby_nearmiss_k = 3,
                          sby_seed = 99L,
                          sby_adasyn_ratio = 1,
                          sby_nearmiss_ratio = 0.5)
  out2 <- sby_adanear_hpc(dat, y ~ ., sby_adasyn_k = 3,
                          sby_nearmiss_k = 3,
                          sby_seed = 99L,
                          sby_adasyn_ratio = 1,
                          sby_nearmiss_ratio = 0.5)
