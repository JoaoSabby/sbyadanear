...
test_that("output of ADASYN has no NA/NaN/Inf even with extreme adasyn_ratio", {
  set.seed(1)
  x <- matrix(rnorm(2000), 1000, 2)
  storage.mode(x) <- "double"
  y <- factor(c(rep("min", 10), rep("maj", 990)), levels = c("min", "maj"))
  r <- sby_adasyn_matrix(x, y, sby_adasyn_ratio = 50, sby_seed = 1L,
                         sby_knn_engine = "FNN")
...
test_that("output of ADANEAR has no NA/NaN/Inf in extreme regimes", {
  ...
  storage.mode(x) <- "double"
  y <- factor(c(rep("min", 10), rep("maj", 990)), levels = c("min", "maj"))
  r <- sby_adanear_matrix(x, y, sby_adasyn_ratio = 5, sby_nearmiss_ratio = 1,
...
test_that("seed makes ADASYN, NearMiss and ADANEAR exactly reproducible", {
  ...
  ada1 <- sby_adasyn_matrix(x, y, sby_adasyn_ratio = 0.5, sby_seed = 42L,
                            sby_knn_engine = "FNN")
  ada2 <- sby_adasyn_matrix(x, y, sby_adasyn_ratio = 0.5, sby_seed = 42L,
                            sby_knn_engine = "FNN")
  expect_identical(ada1$sby_x_matrix, ada2$sby_x_matrix)
  expect_identical(ada1$sby_y_vector, ada2$sby_y_vector)

  near1 <- sby_nearmiss_matrix(x, y, sby_nearmiss_ratio = 0.5, sby_seed = 42L,
                               sby_knn_engine = "FNN")
  near2 <- sby_nearmiss_matrix(x, y, sby_nearmiss_ratio = 0.5, sby_seed = 42L,
                               sby_knn_engine = "FNN")
  expect_identical(near1$sby_x_matrix, near2$sby_x_matrix)
  expect_identical(near1$sby_y_vector, near2$sby_y_vector)

  ad1 <- sby_adanear_matrix(x, y, sby_adasyn_ratio = 0.5, sby_nearmiss_ratio = 0.8,
                            sby_seed = 42L, sby_knn_engine = "FNN")
  ad2 <- sby_adanear_matrix(x, y, sby_adasyn_ratio = 0.5, sby_nearmiss_ratio = 0.8,
                            sby_seed = 42L, sby_knn_engine = "FNN")
...
test_that("tabular API rejects logical, character, and ordered factor sensibly", {
  ...
  expect_s3_class(sby_adasyn(y_char ~ ., dat, sby_adasyn_ratio = 0.5,
                             sby_seed = 1L, sby_knn_engine = "FNN"),
                  "tbl_df")

  dat$y_logical <- c(rep(FALSE, 5), rep(TRUE, 15))
  expect_s3_class(sby_adasyn(y_logical ~ ., dat, sby_adasyn_ratio = 0.5,
                             sby_seed = 1L, sby_knn_engine = "FNN"),
                  "tbl_df")

  dat$y_ord <- factor(c(rep("L", 5), rep("H", 15)),
                      levels = c("L", "H"), ordered = TRUE)
  expect_s3_class(sby_adasyn(y_ord ~ ., dat, sby_adasyn_ratio = 0.5,
                             sby_seed = 1L, sby_knn_engine = "FNN"),
                  "tbl_df")
})
...
test_that("tabular API gives a clear error when TARGET is reused as predictor", {
  ...
  dat <- data.frame(x1 = rnorm(20), TARGET = rnorm(20),
                    y = factor(c(rep("a", 5), rep("b", 15)),
                               levels = c("a", "b")))
  expect_error(sby_adasyn(y ~ ., dat, sby_adasyn_ratio = 0.5, sby_seed = 1L,
                          sby_knn_engine = "FNN"),
               regexp = "TARGET", ignore.case = FALSE)
})

test_that("exclusive zero ratios are rejected and nearmiss_ratio above one is accepted", {
  set.seed(1)
  x <- matrix(rnorm(40), 20, 2)
  storage.mode(x) <- "double"
  y <- factor(c(rep("min", 5), rep("maj", 15)), levels = c("min", "maj"))

  expect_error(sby_adasyn_matrix(x, y, sby_adasyn_ratio = 0, sby_knn_engine = "FNN"),
               regexp = "positivo|positive", ignore.case = TRUE)
  expect_error(sby_nearmiss_matrix(x, y, sby_nearmiss_ratio = 0,
                                   sby_knn_engine = "FNN"),
               regexp = "maior que zero|positive", ignore.case = TRUE)
  expect_error(sby_nearmiss_matrix(x, y, sby_nearmiss_ratio = -1,
                                   sby_knn_engine = "FNN"),
               regexp = "maior que zero|positive", ignore.case = TRUE)
  expect_equal(
    as.integer(table(sby_nearmiss_matrix(x, y, sby_nearmiss_ratio = 2,
...
test_that("adanear zero ratios skip the corresponding stages", {
  ...
  no_over <- sby_adanear_matrix(x, y, sby_adasyn_ratio = 0, sby_nearmiss_ratio = 1,
  ...
  no_under <- sby_adanear_matrix(x, y, sby_adasyn_ratio = 0.2, sby_nearmiss_ratio = 0,
                                 sby_knn_engine = "FNN", sby_return_scaled = TRUE)
  ...
  neither <- sby_adanear_matrix(x, y, sby_adasyn_ratio = 0, sby_nearmiss_ratio = 0,
                                sby_knn_engine = "FNN", sby_return_scaled = TRUE)
...
test_that("k larger than minority count is handled gracefully", {
  ...
  r <- sby_adasyn_matrix(x, y, sby_adasyn_ratio = 0.5, sby_knn_over_k = 50L,
                         sby_seed = 1L, sby_knn_engine = "FNN")
