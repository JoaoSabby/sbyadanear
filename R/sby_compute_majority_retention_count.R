...
#'
#' @param sby_nearmiss_ratio Multiplicador positivo da quantidade minoritaria para definir a maioria retida
#'
...
sby_compute_majority_retention_count <- function(
  ...
  sby_target_majority_count <- floor(sby_minority_count * sby_nearmiss_ratio)
  ...
  if(sby_retained_count < 1L){
    ...
    sby_adanear_abort(
      sby_message = "'sby_nearmiss_ratio' reteve zero linhas"
    )
  }
