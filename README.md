...
## Exemplos com Intel oneDAL

A Intel oneDAL é um backend opcional para a busca KNN exata por força bruta. Ela
é usada nas rotas públicas de balanceamento quando o pacote foi compilado com
suporte a oneDAL e a chamada solicita explicitamente KNN euclidiano exato com
`FNN`, algoritmo `brute` e backend `RcppParallel`. Essa combinação é apropriada
para matrizes densas, numéricas e de dimensão moderada ou alta, nas quais a
consulta KNN domina o custo computacional.

A instalação deve ser feita em um ambiente com Intel oneAPI disponível. Em Linux,
uma configuração típica é:

```sh
source /opt/intel/oneapi/setvars.sh
SBY_USE_ONEDAL=1 R CMD INSTALL .
```

Em Windows, defina `SBY_USE_ONEDAL=1` antes da instalação e garanta que
`ONEAPI_ROOT` ou `DALROOT` aponte para a instalação da oneDAL. Quando a oneDAL
não está disponível no momento da compilação, a mesma chamada permanece válida,
mas o pacote usa o fallback nativo paralelo baseado em `RcppParallel`.

O exemplo abaixo cria uma base pequena apenas para demonstrar a configuração. Em
uso operacional, as variáveis categóricas devem ser codificadas antes da chamada,
os preditores devem ser finitos e numéricos, e a semente deve ser fixada para
reprodutibilidade.
...
sby_onedal_data <- tibble::tibble(
  sby_x1 = rnorm(120),
  sby_x2 = rnorm(120),
  sby_x3 = rnorm(120),
  sby_y = factor(c(rep("minority", 24), rep("majority", 96)))
)

sby_onedal_workers <- max(1L, parallel::detectCores(logical = FALSE) - 1L)
```

### `sby_adanear()` com oneDAL

O pipeline híbrido executa primeiro a sobreamostragem ADASYN e, em seguida, a
subamostragem NearMiss-1. Com `sby_knn_engine = "FNN"`,
`sby_knn_algorithm = "brute"`, `sby_knn_distance_metric = "euclidean"` e
`sby_knn_parallel_backend = "RcppParallel"`, as consultas KNN exatas elegíveis
são encaminhadas à oneDAL quando o pacote foi compilado com esse suporte.

```r
sby_onedal_adanear <- sby_adanear(
  sby_formula = sby_y ~ .,
  sby_data = sby_onedal_data,
  sby_over_ratio = 0.5,
  sby_under_ratio = 0.8,
  sby_knn_over_k = 5L,
  sby_knn_under_k = 5L,
  ...
  sby_knn_distance_metric = "euclidean",
  sby_knn_parallel_backend = "RcppParallel",
  sby_knn_workers = sby_onedal_workers,
  sby_seed = 123,
  sby_audit = TRUE
)

sby_onedal_adanear$sby_diagnostics
sby_onedal_adanear$sby_balanced_data
```

### `sby_adasyn()` com oneDAL

A chamada isolada de ADASYN aumenta adaptativamente a classe minoritária. A
oneDAL acelera as consultas de vizinhança usadas para estimar a dificuldade
local de cada observação minoritária e para gerar amostras sintéticas coerentes
com essa vizinhança.

```r
sby_onedal_adasyn <- sby_adasyn(
  sby_formula = sby_y ~ .,
  sby_data = sby_onedal_data,
  sby_over_ratio = 0.5,
  sby_knn_over_k = 5L,
...
sby_onedal_adasyn$sby_diagnostics
sby_onedal_adasyn$sby_balanced_data
```

### `sby_nearmiss()` com oneDAL

A chamada isolada de NearMiss-1 reduz a classe majoritária mantendo observações
majoritárias mais próximas da classe minoritária. Com a configuração abaixo, a
seleção usa KNN euclidiano exato por força bruta, permitindo que a oneDAL seja
utilizada quando disponível.

```r
sby_onedal_nearmiss <- sby_nearmiss(
  sby_formula = sby_y ~ .,
  sby_data = sby_onedal_data,
  sby_under_ratio = 0.8,
  sby_knn_under_k = 5L,
...
sby_onedal_nearmiss$sby_diagnostics
sby_onedal_nearmiss$sby_balanced_data
```
